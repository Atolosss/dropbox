...
class FileControllerTest extends IntegrationTestBase {
    ...
    void postFileShouldWork() {
        ...
        executeInTransaction(() -> assertThat(fileRepository.findAll())
            ...
            .ignoringFields("id", "fileType", "audit", "user", "key")
    ...
    void getFileShouldWork() {
        ...
        assertThat(getFileByUserId(file.getId()))
            .usingRecursiveComparison()
            .ignoringFields("fileData")
            .isEqualTo(UploadFileDtoRs.builder()
                .name(file.getName()));
    }

    private UploadFileDtoRs getFileByUserId(final Long id) {
        return webTestClient.get()
            ...
            .exchange()
            .expectStatus().isEqualTo(200)
            .expectBody(UploadFileDtoRs.class)
            .returnResult()
            .getResponseBody();
    }
